...
import { decodeState, NONCE_COOKIE } from "@/lib/oauthState";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const tokens = await instagramConnector.handleCallback(code, "", state.brandMode);

    await db.channelConnection.upsert({
      where: { brandMode_platform: { brandMode: state.brandMode as any, platform: "instagram" } },
      create: {
    ...
    console.error("[Instagram callback] error:", e instanceof Error ? e.message : String(e));
    const msg = e instanceof Error ? e.message : "callback_failed";
    return NextResponse.redirect(
      new URL(`${SETTINGS_URL}?error=${encodeURIComponent(msg)}`, req.url)
    );
  }

  const res = NextResponse.redirect(
    new URL(`${SETTINGS_URL}?connected=instagram&brand=${state.brandMode}`, req.url)
  );
