...
export async function GET(req: NextRequest) {
  ...
  try {
    const tokens = await linkedinConnector.handleCallback(code, "");

    await db.channelConnection.upsert({
      where: { brandMode_platform: { brandMode: state.brandMode as any, platform: "linkedin" } },
      create: {
  ...
  const res = NextResponse.redirect(
    new URL(`${SETTINGS_URL}?connected=linkedin&brand=${state.brandMode}`, req.url)
  );
