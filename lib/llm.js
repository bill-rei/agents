async function compile(userMessage, systemPrompt, opts = {}) {
  if (!systemPrompt) {
  ...
  if (provider === 'anthropic') {
    ...
    return compileGrok(userMessage, systemPrompt);
  } else {
    throw new Error(`Unknown LLM_PROVIDER: "${provider}". Use "anthropic", "openai", or "grok".`);
  }
}
...
async function compileGrok(userMessage, systemPrompt) {
  const OpenAI = require('openai');
  const apiKey = process.env.XAI_API_KEY;
  if (!apiKey) {
    throw new Error(
      'Missing xAI API key. Set XAI_API_KEY in the environment. [PROVIDER_NOT_CONFIGURED]'
    );
  }

  // XAI_API_BASE_URL may be set without /v1 (as in .env.example); normalise it.
  const baseURL =
    (process.env.XAI_API_BASE_URL || 'https://api.x.ai').replace(/\/v1\/?$/, '') + '/v1';
